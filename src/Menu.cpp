...
namespace FootballManagement
{
    ...
    void Menu::LoadAllData() const
    {
        ...
        try
        {
            const auto playerLines = fileManager_.LoadFromFile(
                PLAYERS_DATA_FILE);
        ...
        while (true)
        {
            ...
            else if (cmd == 2)
            {
                ...
                const std::string password = InputValidator::GetNonEmptyString(
                    "Пароль (≥6 символів): ");
                const int roleInt = InputValidator::GetIntInRange(
                    "Роль (1=User, 0=Admin): ", 0, 1);
                const UserRole role = (roleInt == 0
                                           ? UserRole::Admin
                                           : UserRole::StandardUser);

                if (AuthManager::GetInstance().Register(login, password, role))
                {
                    fileManager_.SaveToFile(USERS_FILE_NAME,
    ...
    {
        std::cout << "\n--- ДОДАВАННЯ ГРАВЦЯ ---\n";
        std::string name = InputValidator::GetNonEmptyString(
            "Ім'я (0 - назад): ");
        if (name == "0") return;
        ...
        if (type == 1)
        {
            ...
            std::string contractUntil =
                InputValidator::GetNonEmptyString(
                    "Контракт до (YYYY-MM-DD) (0 - назад): ");
    ...
    void Menu::editPlayerFlow()
    {
        ...
        case 1:
            {
                std::string newName = InputValidator::GetNonEmptyString(
                    "Нове ім’я: ");
        ...
        case 3:
            {
                ...
                player->SetNationality(newNat);
                break;
            }
        case 4:
            {
                double newVal = InputValidator::GetDoubleInput(
                    "Нова ринкова вартість: ");
                player->SetMarketValue(newVal);
                break;
            }
        default: return;
        }
    ...
    {
        std::string raw = InputValidator::GetNonEmptyString(
            "ID гравця (0 - назад): ");
        if (raw == "0") return;
    ...
    void Menu::searchPlayerFlow() const
    {
        while (true)
        {
            ...
            {
                std::string st = InputValidator::GetNonEmptyString(
                    "Статус (0 - назад): ");
                if (st == "0") continue;
    ...
    void Menu::adminCreateUser()
    {
        ...
        int role = InputValidator::GetIntInRange(
            "Роль (1=Admin, 2=User, 0=Назад): ", 0, 2);
        if (role == 0) return;
        ...
        {
            fileManager_.
                SaveToFile(USERS_FILE_NAME, AuthManager::GetInstance());
            std::cout << "[УСПІХ] Користувача створено.\n";
        }
        else
        {
            std::cout <<
                "[ПОМИЛКА] Не вдалося створити (логін може існувати).\n";
        }
    }
    ...
    void Menu::EditPlayerStatsFlow()
    {
        ...
        if (auto gk = std::dynamic_pointer_cast<Goalkeeper>(base))
        {
            ...
            int cleanSheets = InputValidator::GetIntInRange(
                "«Сухі» матчі: ", 0, 2000);
            int conceded = InputValidator::GetIntInRange(
                "Пропущені голи: ", 0, 2000);
            ...
            std::cout <<
                "[ІНФО] Стати GK введені. Під’єднайте сеттери у Goalkeeper.\n";
            SaveAllData();
            return;
        }
        ...
        {
            std::cout <<
                "[ПОПЕРЕДЖЕННЯ] Тип гравця не підтримано для редагування статів.\n";
            return;
        }

        std::cout <<
            "[ІНФО] Стати польового введені. Під’єднайте сеттери у похідних.\n";
        SaveAllData();
    }
}
