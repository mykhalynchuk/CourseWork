...
namespace FootballManagement
{
    ...
    bool ContractDetails::ValidateIsoDate(const std::string& s)
    {
        ...
        if (d < 1 || d > 31) return false; // спрощено
        return true;
    }
          ...
          loanEndDate_("")
    {
        ...
        if (!ValidateIsoDate(contractUntil_))
            throw std::invalid_argument("Невірний формат дати (очікується YYYY-MM-DD).");
    }
    ...
    ContractDetails& ContractDetails::operator=(const ContractDetails& other) = default;

    ContractDetails& ContractDetails::operator=(ContractDetails&& other) noexcept
    {
        if (this != &other)
        {
            clubName_      = std::move(other.clubName_);
    ...
    std::string ContractDetails::GetContractUntil() const { return contractUntil_; }
    ...
    void ContractDetails::SetOnLoan(const std::string& loanEndDate)
    {
        if (!ValidateIsoDate(loanEndDate))
            throw std::invalid_argument("Дата завершення оренди має формат YYYY-MM-DD.");
    ...
    {
        if (salary_ <= 0.0)
            throw std::logic_error("Неможливо змінити зарплату: поточне значення ≤ 0.");
